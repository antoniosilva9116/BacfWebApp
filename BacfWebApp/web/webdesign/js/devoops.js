...
function LoadCalendarScript(callback) {
    function LoadFullCalendarScript() {
        if (!$.fn.fullCalendar) {
            $.getScript('../webdesign/plugins/fullcalendar/fullcalendar.js', callback);
        }
        else {
    ...
    if (!$.fn.moment) {
        $.getScript('../webdesign/plugins/moment/moment.min.js', LoadFullCalendarScript);
    }
    else {
...
function LoadTimePickerScript(callback) {
    if (!$.fn.timepicker) {
        $.getScript('../webdesign/plugins/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.js', callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadBootstrapValidatorScript(callback) {
    if (!$.fn.bootstrapValidator) {
        $.getScript('../webdesign/plugins/bootstrapvalidator/bootstrapValidator.min.js', callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadSelect2Script(callback) {
    if (!$.fn.select2) {
        $.getScript('../webdesign/plugins/select2/select2.min.js', callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadDataTablesScripts(callback) {
    function LoadDatatables() {
        $.getScript('../webdesign/plugins/datatables/jquery.dataTables.js', function() {
            $.getScript('../webdesign/plugins/datatables/ZeroClipboard.js', function() {
                $.getScript('../webdesign/plugins/datatables/TableTools.js', function() {
                    $.getScript('../webdesign/plugins/datatables/dataTables.bootstrap.js', callback);
                });
            });
        });
    }
...
function LoadFineUploader(callback) {
    if (!$.fn.fineuploader) {
        $.getScript('../webdesign/plugins/fineuploader/jquery.fineuploader-4.3.1.min.js', callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadXChartScript(callback) {
    function LoadXChart() {
        $.getScript('../webdesign/plugins/xcharts/xcharts.min.js', callback);
    }
    function LoadD3Script() {
        if (!$.fn.d3) {
            $.getScript('../webdesign/plugins/d3/d3.v3.min.js', LoadXChart)
        }
        else {
            LoadXChart();
        }
...
function LoadFlotScripts(callback) {
    function LoadFlotScript() {
        $.getScript('../webdesign/plugins/flot/jquery.flot.js', LoadFlotResizeScript);
    }
    function LoadFlotResizeScript() {
        $.getScript('../webdesign/plugins/flot/jquery.flot.resize.js', LoadFlotTimeScript);
    }
    function LoadFlotTimeScript() {
        $.getScript('../webdesign/plugins/flot/jquery.flot.time.js', callback);
    }
    if (!$.fn.flot) {
        LoadFlotScript();
    }
...
function LoadMorrisScripts(callback) {
    function LoadMorrisScript() {
        if (!$.fn.Morris) {
            $.getScript('../webdesign/plugins/morris/morris.min.js', callback);
        }
        else {
            if (callback && typeof (callback) === "function") {
                callback();
    ...
    if (!$.fn.raphael) {
        $.getScript('../webdesign/plugins/raphael/raphael-min.js', LoadMorrisScript);
    }
    else {
        LoadMorrisScript();
    }
...
function LoadFancyboxScript(callback) {
    if (!$.fn.fancybox) {
        $.getScript('../webdesign/plugins/fancybox/jquery.fancybox.js', callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadKnobScripts(callback) {
    if (!$.fn.knob) {
        $.getScript('../webdesign/plugins/jQuery-Knob/jquery.knob.js', callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadSparkLineScript(callback) {
    if (!$.fn.sparkline) {
        $.getScript('../webdesign/plugins/sparkline/jquery.sparkline.min.js', callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
        ...
        yaxes: [
            {min: 0},
            {
                ...
                tickFormatter: function(value, axis) {
                    return value.toFixed(axis.tickDecimals) + "ï¿½";
                }
            }
        ],
        legend: {position: "sw"}
    ...
    var calendar = $('#calendar').fullCalendar({
        ...
        select: function(start, end, allDay) {
            ...
            var form = $('<form id="event_form">' +
                    ...
                    '<div>' +
                    '<input type="text" id="newevent_name" class="form-control" p:placeholder="Name of event">' +
                    '</div>' +
            ...
            var buttons = $('<button id="event_cancel" type="cancel" class="btn btn-default btn-label-left">' +
                    ...
                    'Cancel' +
                    '</button>' +
                    '<button onclick="CallWebServiceCreateCampanha()" type="submit" id="event_submit"  class="btn btn-primary btn-label-left pull-right">' +
...
$(document).ready(function() {
    ...
    if (ajax_url.length < 1) {
        ajax_url = 'perfil.xhtml';
    }
    var path = (window.location.pathname).split('/');
    var name = path[path.length-1];
    if (!isPostBack && name !== "administrator1.xhtml" && name !== "estatisticas.xhtml")
    ...
    $('.main-menu').on('click', 'a', function(e) {
        ...
        if ($(this).hasClass('ajax-link')) {
            ...
            var urlName = $(this).attr('href').replace("../", "");
            var userID = $(this).attr('target');

            if ("estatisticas.xhtml" == (urlName))
                countUsers();

            var aux = urlName.split('/');
            var urlName = aux[aux.length - 1];
            var url = $(this).attr('href');
            window.location.hash = urlName;
            LoadAjaxContent(url);
            if (userID)
                CallWebServiceGetEmails(userID);
        }
        if ($(this).attr('href') == '#') {
            e.preventDefault();
        }
